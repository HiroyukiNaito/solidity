...
依然として、v0.5.0以前のSolidityバージョンで書かれたコントラクトとインターフェースを定義することで
(相互に)やり取りを行うことが可能です。
既にデプロイされている以下のような0.5.0より前のバージョンのコントラクトがあるとします。
...
このコントラクトは、Solidity v0.5.0ではコンパイルすることができません。しかし、以下のように互換性のあるインターフェースを定義できます。
...
v0.5.0より前のバージョンでは、``constant``キーワードが強制されていなかったため、``constant``で宣言された関数を``staticcall``で呼び出すとリバートします。
...
リンク時に0.5.0よりも前のバージョンのライブラリアドレスを提供します
(コマンドラインコンパイラでリンクする方法は、:ref:`コマンドラインコンパイラ<commandline-compiler>`を参照してください)。
